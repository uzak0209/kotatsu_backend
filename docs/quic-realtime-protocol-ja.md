...
4. クライアントは `quic_url` に QUIC 接続する
5. 接続後、最初に Reliable stream で `join` メッセージを送る
...
ホストが HTTP `POST /v1/matches/{match_id}/start` を呼ぶと、同じマッチ内の接続済みプレイヤーへ broadcast されます。
...
4. `quic_url` に QUIC 接続
5. bidirectional stream を開く
6. Reliable stream で `{"t":"join","token":"..."}` を送る
7. `join_ok` を受け取る
8. 開始後は `match_started` を受け取る
9. 必要に応じて:
